...
- 現在は `user` が取れれば `subscribeToRecords` を開始する。
- Googleログイン（`user.email` あり）のときは Firestore を購読。
- Firebase 未設定 or 匿名相当のときは `localStorage`（`pachislo-records-v9`）を購読し、ローカル保存モードで動作する。
...
- 進捗: 一部完了（`AppSections.jsx` を新設して UI サブコンポーネントを分離）。
...
- Firebase 未設定でもローカル保存モードで記録できるように修正。
- `直近履歴 -> 編集 -> 修正を保存` 後に期間フィルター以外が消える不具合を修正（編集前タブへ復帰）。
- 貸玉レートボタンが意図せずフォーム送信していた不具合を修正。
- モバイル実機相当のスモークテスト（`scripts/mobile-smoke.mjs`）を追加。
- 機種選択に `テスト` を追加し、保存確認を自動テストへ組み込み。
...
## 運用ルール
- 通常作業は `main` へ直接コミットせず、内容に応じて `feature/...` `fix/...` `chore/...` `refactor/...` などの作業ブランチを切って進める。
- GitHub Pages は `main` への push で更新されるため、試作・検証段階の変更は作業ブランチで確認し、反映判断後に `main` へ取り込む。
- プッシュ前に、今回の変更で増えた重複処理・条件分岐・責務過多がないかを確認し、必要なら小さなリファクタリングを先に挟む。
- リファクタリングは「挙動を変えない整理」を原則とし、変更後は少なくとも `npm run test` と必要な確認（例: モバイルUI変更時は `npm run test:mobile`）を通してからプッシュする。
